...
class CustomDataset(MyDataset):
    ...
    def get_data(self, train_test = 'train'):
        try:
            codes = torch.load(os.path.join(self.data_dir, f"{train_test}_data.pt"))
        except FileNotFoundError:
            if train_test=='test':
                print(f"Test data not found! running only with train data")
        ...
        if type(codes).__module__ == np.__name__ :
        ...
        if self.args.use_labels_for_eval:
            labels = torch.Tensor(torch.load(os.path.join(self.data_dir, f"{train_test}_labels.pt")))
